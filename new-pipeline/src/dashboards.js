...
// Generate a map of arrays of individual filter sets that,
// when the resulting histograms are added together,
// results in the same histogram as if all the filter options were selected
function getFilterSetsMapping(filters, comparisonName) {
  ...
  for (var filterName in filters) {
    // Some options are not selected, so we need to explicitly filter
    var selector = filters[filterName];
    var selected = selector.val() || [];
    if (selected.length !== selector.find("option").length) {
        ...
        return filterSets.map(function(filterSet) {
          ...
          if (filterName === "os") {
            if (filterValue.indexOf(",") >= 0) {
              // We have an OS value like "Windows_NT,6.1", and we want to set the
              // os filter to Windows_NT and the osVersion filter to 6.1
              var parts = filterValue.split(",");
              newFilterSet["os"] = parts[0]; newFilterSet["osVersion"] = parts[1];
            } else {
              // We have an OS value like "Windows_NT" (all versions of Windows),
              // just set the os filter, not the osVersion filter
