...
$(function() { Telemetry.init(function() {
  ...
  updateOptions(function() {
    ...
    $([
      ...
      "#filter-e10s", "#filter-process-type", "#compare",
      ...
      gFilterChangeTimeout = setTimeout(function() {
        ...
        calculateHistograms(function(histogramsMap, evolutionsMap) {
          ...
          // (one per comparison option, so each histogram in a list has the same buckets)
          // evolutionsMap is a mapping from keyed histogram keys
          // (or "" if not a keyed histogram) to lists of evolutions
          // (one per comparison option, so each evolution in a list has the same dates)

          // Get the set union of all the dates in all the evolutions
          var datesMap = {};
          for (var label in evolutionsMap) {
            evolutionsMap[label][0].dates().forEach(function(date) {
          ...
          // Set up key selectors, selecting the previously selected key
          // if it still exists and the first key otherwise
          var getAggregate = { // Function to get an aggregate for a list of histograms, used for sorting later
            "submissions": function(histograms) {
              return histograms.reduce(function(total, histogram) {
            ...
            "5th-percentile": function(histograms) {
              return histograms.reduce(function(total, histogram) {
                return total + histogram.percentile(5);
              }, 0) / histograms.length;
            },
            "25th-percentile": function(histograms) {
              return histograms.reduce(function(total, histogram) {
                return total + histogram.percentile(25);
              }, 0) / histograms.length;
            },
            "median": function(histograms) {
              return histograms.reduce(function(total, histogram) {
                return total + histogram.percentile(50);
              }, 0) / histograms.length;
            },
            "75th-percentile": function(histograms) {
              return histograms.reduce(function(total, histogram) {
                return total + histogram.percentile(75);
              }, 0) / histograms.length;
            },
            "95th-percentile": function(histograms) {
              return histograms.reduce(function(total, histogram) {
                return total + histogram.percentile(95);
              }, 0) / histograms.length;
            },
          }[$("#sort-keys").val()];
          ...
          if (gInitialPageState.keys) { // Reselect previously selected keys
            gInitialPageState.keys.forEach(function(key, i) {
              ...
              var hasKey = false;
              gAxesSelectors[i].find("option").each(function(i, option) {
    ...
    $([
      ...
      // Keyed histogram with multiple keys, find the selected keys
...
function updateOptions(callback) {
  ...
  Telemetry.getFilterOptions(parts[0], parts[1], function(optionsMap) {
    ...
    // Compressing and expanding the OSs also has the effect of making OSs
    // where all the versions were selected also all selected in the new one,
...
function calculateHistograms(callback, sanitize) {
  ...
  // No filters selected, or no measures available, so no histograms could be created
  if (totalFilters === 0 || measure === null) {
    indicate();
  ...
  filterSetsMappingOptions.forEach(function(filterSetsMappingOption, i) {
    ...
    filterSets.forEach(function(filterSet) {
      ...
      Telemetry.getEvolution(
        parts[0], parts[1], measure, filterSet, useSubmissionDate,
...
function processHistograms(fullEvolutionsMap, optionValues, comparisonName, callback) {
  // Get the set union of all the dates in all the evolutions
  var datesMap = {};
  for (var label in fullEvolutionsMap) {
    fullEvolutionsMap[label][0].dates()
      .forEach(function(date) { datesMap[date.getTime()] = true; });
  }
  ...
  updateDateRange(function(dates) {
    if (dates == null) {
      // No dates in the selected range, so no histograms available
      callback({}, {});
    } else {
      // Filter the evolution to include only those histograms that are in the selected range
      var filteredEvolutionsMap = {}, filteredHistogramsMap = {};
      for (var label in fullEvolutionsMap) {
        var filteredEntries = fullEvolutionsMap[label].map(function(evolution, i) {
          // We don't need to worry about this returning null since the dates
          // came from the evolution originally
          return {
            option: optionValues[label][i],
            evolution: evolution.dateRange(dates[0], dates[dates.length - 1]),
          };
        ...
        if (filteredEvolutions.length > 0) { // There are evolutions in this date
          ...
          filteredHistogramsMap[label] = filteredEvolutions.map(function(evolution, i) {
            ...
            if (comparisonName !== "") { // We are comparing by an option value
              var humanReadableOption = getHumanReadableOptions(
                comparisonName,
                [filteredOptionValues[i]]
              )[0][1];
...
// dates is null for when there are no evolutions
...
function updateDateRange(callback, dates, updatedByUser, shouldUpdateRangebar) {
  ...
  if (shouldLoadDateRange) {
    ...
    var advancedSettingsModified = gInitialPageState.use_submission_date !== 0 ||
      ...
      gInitialPageState.sort_keys !== "submissions" ||
...
function displayHistograms(histogramsList, dates, useTable, cumulative, trim) {
  ...
  // Find the largest percentage on the graph to determine the upper bound for all the graphs
  histogramsList.forEach(function(entry) {
    if (cumulative) {
      maxPercentage = 100; // Cumulative histograms always have a max percentage of 100%
    } else {
  ...
  // Figure out how much to trim buckets on both ends in the histogram if their counts are too low
  if (trim) {
    // Histograms must have at least 3 buckets to render properly,
    // so ensure that we don't trim them too much
    ...
    histogramsList.forEach(function(entry) {
      ...
      while (
        countList[trimLeft] < countCutoff &&
        countList.length - trimLeft - trimRight > 3
      ) {
        trimLeft ++;
      }
      while (
        countList[countList.length - 1 - trimRight] < countCutoff &&
        countList.length - trimLeft - trimRight > 3
      ) {
        trimRight ++;
      }
  ...
  if (histogramsList.length <= 1) { // Only one histograms set
    ...
    if (histogramsList.length > 0) {
      displaySingleHistogramSet(
        ...
        histogramsList[0].histograms, histogramsList[0].title,
...
function displaySingleHistogramSet(axes, useTable, histograms, title, cumulative, trimLeft, trimRight, max) {
  ...
  // Display the histogram as a table rather than a chart
  if (useTable) {
  ...
  if (histograms.length === 1) { // One histogram available, display as histogram
    ...
    MG.data_graphic({
      ...
      full_width: true, height: $(axes).width() * 0.6,
      left: 70,
      max_y: max + 2, // Add some extra space to account for the bezier curves
      transition_on_update: false,
      target: axes,
      ...
      mouseover: function(d, i) {
        ...
        if (d.values) {
          ...
          rolloverCircle = $(axes).find(".mg-line-rollover-circle.mg-area" + line_id + "-color").get(0);
        ...
        var padding = 10;
        d3.select(axes).select(".active-datapoint-background").remove(); // Remove old background
        d3.select(axes).select("svg").insert("rect", ".mg-active-datapoint-container")
          .classed("active-datapoint-background", true)
    ...
    countsList.forEach(function(counts, i) { // Recolor lines with the previously calculated colors
      ...
      $(axes).find(".mg-area" + lineIndex + "-color, .mg-hover-line" + lineIndex + "-color")
        .css("fill", colors[i]).css("stroke", colors[i]);
...
function saveStateToUrlAndCookie() {
  ...
  if (gCurrentHistogramsList.length > 0 && gCurrentHistogramsList[0].histograms.length === 1) {
    ...
    $("#export-csv").attr("href", gPreviousCSVBlobUrl)
      .attr("download", histogram.measure + ".csv").show();
    $("#export-json").attr("href", gPreviousJSONBlobUrl)
      .attr("download", histogram.measure + ".json").show();
  } else {
