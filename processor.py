        ...
        if type(value) == dict:
            for subName, subValue in value.iteritems():
                map_simplemeasure(channel, majorVersion, filterPath,
                                  name + "_" + str(subName), subValue)
        else:
            map_simplemeasure(channel, majorVersion, filterPath, str(name),
                              value)

# Map a simple measure
def map_simplemeasure(channel, majorVersion, filterPath, name, value):
    # Sanity check value
...
class Processor:
    ...
    def write_output(self):
        f = open(os.path.join(self.output_folder, "result.txt"), "w")
        global cache
