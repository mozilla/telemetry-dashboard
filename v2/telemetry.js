...
Telemetry.Histogram = (function() {
  function Histogram(buckets, values, kind, submissions, sum, description, measure) {
    ...
    assert(
      ["flag", "boolean", "count", "enumerated", "linear", "exponential"].indexOf(kind) >= 0,
  ...
  Histogram.prototype.lastBucketUpper = function() {
    ...
    if (["linear", "flag", "boolean", "enumerated"].indexOf(this.kind) >= 0) { // linear buckets
      return this.buckets[this.buckets.length - 1] +
             (this.buckets[this.buckets.length - 1] - this.buckets[this.buckets.length - 2]);
  ...
  Histogram.prototype.mean = function() {
    ...
    var useLinearBuckets = ["linear", "flag", "boolean", "enumerated"].indexOf(this.kind) >= 0;
    this.values.forEach(function(count, i) {
      totalHits += count;
      var centralX = useLinearBuckets ? buckets[i] + linearTerm : buckets[i] * exponentialFactor;
  ...
  Histogram.prototype.percentile = function(percentile) {
    ...
    var hitsAtPercentileInBar = this.values
      .reduce(function(previous, count) { return previous + count; }, 0) * (percentile / 100);
    ...
    percentileBucketIndex --; // decrement to get to the bar containing the percentile
    hitsAtPercentileInBar += this.values[percentileBucketIndex];

    // the ratio of the hits in the percentile to the hits in the bar containing it,
    // or how far we are inside the bar
    var ratioInBar = hitsAtPercentileInBar / this.values[percentileBucketIndex];
    if (["linear", "flag", "boolean", "enumerated"].indexOf(this.kind) >= 0) { // linear buckets
      // linear interpolation within bar
      return buckets[percentileBucketIndex] + linearTerm * ratioInBar;
    } else { // exponential buckets
      // geometric interpolation within bar
...
Telemetry.Evolution = (function() {
  ...
  Evolution.prototype.dates = function() {
    return this.data.map(function(entry) {
      ...
      var MM = entry.date.substring(4, 6);
      var DD = entry.date.substring(6, 8);
  ...
  Evolution.prototype.combine = function(otherEvolution) {
    ...
    assert(
      this.buckets.length === otherEvolution.buckets.length,
    ...
    var data = Object.keys(dateMap).sort().map(function(date) {
      // go through each histogram entry and combine histograms
    ...
    return new Telemetry.Evolution(
      this.buckets, data,
      this.kind, this.description, this.measure
    );
  };
  ...
  Evolution.prototype.sanitized = function() {
    ...
    // Set time cutoff to 100 days in the future
    var timeCutoff = new Date;
    timeCutoff.setDate(timeCutoff.getDate() + 100);

    // Remove entries that have too few submissions, or are too far into the future
  ...
  Evolution.prototype.histogram = function() {
    var submissions = this.data.reduce(function(sum, entry) { return sum + entry.count; }, 0);
    ...
    return new Telemetry.Histogram(
      this.buckets, values,
      this.kind, submissions, sum, this.description, this.measure
    );
  };
  ...
  Evolution.prototype.map = function(callback) {
    ...
    return this.data.sort(function(a, b) { return parseInt(a.date) - parseInt(b.date); })
      ...
      var histogram = new Telemetry.Histogram(
        evolution.buckets, entry.histogram,
        evolution.kind, entry.count, entry.sum, evolution.description, evolution.measure
      );
...
Telemetry.getJSON = function(url, callback) {
  ...
  if (Telemetry.CACHE[url] !== undefined) {
    if (Telemetry.CACHE[url] !== null && Telemetry.CACHE[url]._loading) {
      // Requested but not yet loaded, add handler to request
      ...
      return;
    } else if ((new Date).getTime() - Telemetry.CACHE_LAST_UPDATED[url] < Telemetry.CACHE_TIMEOUT) {
      // In cache and hasn't expired, produce cached result
      setTimeout(function() {
  ...
  xhr.onload = function() {
    if (this.status !== 200) {
      if (this.status === 404) { // URL resolves to a resource or missing resource, cache null result
...
Telemetry.init = function Telemetry_init(callback) {
  ...
  Telemetry.getJSON(Telemetry.BASE_URL + "aggregates_by/build_id/channels/", function(channels) {
    ...
    channels.forEach(function(channel, i) {
      ...
      Telemetry.getJSON(
        Telemetry.BASE_URL + "aggregates_by/build_id/channels/" + channel + "/dates/",
        function(buildIdEntries) {
          ...
          // Check if all channels have been loaded
          loadedChannels ++; // Loaded another channel's dates
          if (loadedChannels == expectedChannels) { callback(); }
        }
      );

      var versionDates = Telemetry.CHANNEL_VERSION_DATES[channel] = {};
      Telemetry.getJSON(
        Telemetry.BASE_URL + "aggregates_by/submission_date/channels/" + channel + "/dates/",
...
Telemetry.getHistogramInfo = function Telemetry_getHistogramInfo(channel, version, metric, useSubmissionDate, callback) {
  assert(
    Telemetry.CHANNEL_VERSION_DATES !== null && Telemetry.CHANNEL_VERSION_BUILDIDS !== null,
    "Telemetry.js must be initialized before use"
  );
  ...
  assert(typeof callback === "function", "`callback` must be a function");
  var datesMap = useSubmissionDate ? Telemetry.CHANNEL_VERSION_DATES : Telemetry.CHANNEL_VERSION_BUILDIDS;
  var dates = datesMap[channel][version];
  var buildDate = dates[0];
  Telemetry.getJSON(
    Telemetry.BASE_URL + "aggregates_by/" + (useSubmissionDate ? "submission_date" : "build_id") +
    "/channels/" + encodeURIComponent(channel) + "/?version=" + encodeURIComponent(version) +
    "&dates=" + buildDate + "&metric=" + encodeURIComponent(metric),
...
Telemetry.getEvolution = function Telemetry_getEvolution(channel, version, metric, filters, useSubmissionDate, callback) {
  ...
  assert(typeof callback === "function", "`callback` must be a function");
  var datesMap = useSubmissionDate ? Telemetry.CHANNEL_VERSION_DATES : Telemetry.CHANNEL_VERSION_BUILDIDS;
  var dates = datesMap[channel][version].join(",");
  var filterString = "";

  // We need to sort the keys in order to make sure the same filters result in the same URL each time,
  // in order to make caching always work
  ...
  Telemetry.getJSON(
    ...
    encodeURIComponent(version) + "&dates=" + encodeURIComponent(dates) +
    "&metric=" + encodeURIComponent(metric) + filterString,
    function(histograms, status) {
      if (histograms === null) {
        // Only allow null evolution if it is 404 - if there is no evolution for the given filters
        assert(status === 404, "Could not obtain evolution: status " + status + " (" + url + ")");
        callback({});
      } else {
        // Create mapping from entry labels to a list of entries having that label
        var entriesMap = {};
        ...
        // Create mapping from labels to evolutions for those labels
        var evolutionMap = {};
        for (var label in entriesMap) {
          evolutionMap[label] = new Telemetry.Evolution(
            histograms.buckets, entriesMap[label],
...
Telemetry.getFilterOptions = function Telemetry_getFilterOptions(channel, version, callback) {
  ...
  assert(typeof callback === "function", "`callback` must be a function");
  Telemetry.getJSON(
    Telemetry.BASE_URL + "filters/?channel=" + encodeURIComponent(channel) +
    "&version=" + encodeURIComponent(version),
    function(filterOptions, status) {
      if (filterOptions === null) {
        // Only allow no filter options if it is 404 - if there are no filters
        assert(status === 404, "Could not obtain filter options: status " + status + " (" + url + ")");
        callback({});
      } else {
...
Telemetry.getVersions = function Telemetry_getVersions(fromVersion, toVersion) { // shim function
  ...
  assert(
    ...
    "`fromVersion` and `toVersion` must be strings or omitted"
  );
  var versions = [];
  for (var channel in Telemetry.CHANNEL_VERSION_DATES) {
    for (var version in Telemetry.CHANNEL_VERSION_DATES[channel]) {
      // Only output a channel/version if it is in both the buildID versions and
      // the submission date versions (sometimes there is invalid data where a version
      // is in submission date versions but not buildID versions,
      // see https://bugzilla.mozilla.org/show_bug.cgi?id=1189727 for details)
      if (Telemetry.CHANNEL_VERSION_BUILDIDS[channel]) {
        if (Telemetry.CHANNEL_VERSION_BUILDIDS[channel][version]) {
