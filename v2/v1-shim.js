(function (exports) {
  ...
  */
  ...
  console.log(
    "WARNING: This library is a shim on top of telemetry.js v2. Upgrading to the telemetry.js v2 API is highly recommended."
  );
  ...
  function getEvolutionSync(Telemetry, channel, version, metric, filters,
    useSubmissionDate, dateList) {
    var newFilters = {};
    if (filters.hasOwnProperty("appName")) {
      newFilters["application"] = filters["appName"];
    }
    if (filters.hasOwnProperty("OS")) {
      newFilters["os"] = filters["OS"];
    }
    if (filters.hasOwnProperty("osVersion")) {
      newFilters["osVersion"] = filters["osVersion"];
    }
    if (filters.hasOwnProperty("arch")) {
      newFilters["architecture"] = filters["arch"];
    }
    ...
    function getJSONSync(url, callback) {
      if (Telemetry.CACHE[url] !== undefined) {
        if (Telemetry.CACHE[url] !== null && Telemetry.CACHE[url]._loading) { // Requested but not yet loaded
          ...
          callback(Telemetry.CACHE[url], Telemetry.CACHE[url] === null ? 404 :
            null);
          return;
        }
      }
    ...
    var dates = (useSubmissionDate ? Telemetry.CHANNEL_VERSION_DATES :
      Telemetry.CHANNEL_VERSION_BUILDIDS)[channel][version].join(","); // Request all the dates in order to take advantage of caching
    var filterString = "";
    Object.keys(newFilters)
      ...
      .forEach(function (filterName) { // we need to sort the keys in order to make sure the same filters result in the same URL each time, for caching
        filterString += "&" + encodeURIComponent(filterName) + "=" +
          encodeURIComponent(newFilters[filterName]);
      });
    var variable = useSubmissionDate ? "submission_date" : "build_id";
    getJSONSync(Telemetry.BASE_URL + "aggregates_by/" + variable +
      "/channels/" + channel +
      "/?version=" + encodeURIComponent(version) + "&dates=" +
      encodeURIComponent(dates) +
      "&metric=" + encodeURIComponent(metric) + filterString,
      function (histograms, status) {
        if (histograms === null) {
          assert(status === 404, "Could not obtain evolution: status " +
            status); // Only allow null evolution if it is 404 - if there is no evolution for the given filters
          evolution = null;
        } else {
          ...
          if (entries.length === 0) { // Weird hack - most v2 dashboards assume that every valid filter configuration has at least one submission, so if there actually aren't any, we put in a fake entry
            entries.push({
              count: 0,
              date: "20000101",
              histogram: histograms.buckets,
              label: "",
              sum: 0
            });
          }
          evolution = new Telemetry.Evolution(histograms.buckets, entries,
            histograms.kind, histograms.description, metric);
        }
      });
    return evolution;
  }

  TelemetryShim.prototype.init = function (callback) {
    this._Telemetry.init(callback);
  }

  TelemetryShim.prototype.doAsync = function (action, thisValue, args,
    callback) {
    setTimeout(function () {
      switch (action) {
  ...
  TelemetryShim.prototype.getHistogramInfo = function (channel, version,
    metric, callback) {
    this._Telemetry.getHistogramInfo(channel, version, metric, callback);
  }

  TelemetryShim.prototype.versions = function () {
    return this._Telemetry.getVersions();
  }

  TelemetryShim.prototype.measures = function (channelVersion, callback) {
    var parts = channelVersion.split("/");
    return this._Telemetry.getFilterOptions(parts[0], parts[1], function (
      filterOptions) {
      var measures = filterOptions.metric || [];
      var measureMap = {};
      measures.forEach(function (measure) {
        measureMap[measure] = null;
      });
      callback(measureMap);
    });
  }

  TelemetryShim.prototype.loadEvolutionOverBuilds = function (channelVersion,
    measure, callback) {
    return this.loadEvolution(channelVersion, measure, false, callback);
  }
  TelemetryShim.prototype.loadEvolutionOverTime = function (channelVersion,
    measure, callback) {
    return this.loadEvolution(channelVersion, measure, false, callback);
  }
  TelemetryShim.prototype.loadEvolution = function Telemetry_loadEvolution(
    channelVersion, measure, useSubmissionDate, callback) {
    var parts = channelVersion.split("/");
    ...
    Telemetry.getFilterOptions(parts[0], parts[1], function (filterOptions) {
      ...
      deduplicate(filterOptions["os"].map(function (option) {
      ...
      Telemetry.getHistogramInfo(parts[0], parts[1], measure,
        ...
        function (kind, description, buckets, dates) {
          var evolution = new TelemetryShim.HistogramEvolutionShim(
            Telemetry, parts[0], parts[1], measure,
            useSubmissionDate, kind, description, dates,
            FILTER_ORDER, filterOptionsList, {});
          callback(evolution);
        });
    });
  }
  ...
  TelemetryShim.HistogramEvolutionShim = (function () {
    function HistogramEvolutionShim(Telemetry, channel, version, measure,
      useSubmissionDate, kind, description, dates, filterNames,
      ...
      for (var i = 0; i < FILTER_ORDER.length && filters.hasOwnProperty(
          FILTER_ORDER[i]); i++) {
        this._filter_path.push(filters[FILTER_ORDER[i]]);
      }
    }

    HistogramEvolutionShim.prototype.measure = function () {
      return this._measure;
    }
    HistogramEvolutionShim.prototype.kind = function () {
      return this._kind;
    }
    HistogramEvolutionShim.prototype.description = function () {
      return this._description;
    }
    HistogramEvolutionShim.prototype.filterName = function () {
      return this._filterNames[0] || null;
    };
    HistogramEvolutionShim.prototype.filterOptions = function () {
      return (this._filterOptionsList[0] || [])
        .slice();
    };
    ...
    HistogramEvolutionShim.prototype.filter = function (option) {
      ...
      for (var filterName in this._filters) {
        newFilters[filterName] = this._filters[filterName];
      }; // Clone the current filters
      newFilters[this.filterName()] = option; // Add the new filter
      return new HistogramEvolutionShim(this._Telemetry, this._channel,
        this._version, this.measure(), this._useSubmissionDate, this.kind(),
        this.description(), this._dates, this._filterNames.slice(1),
        this._filterOptionsList.slice(1), newFilters);
    };
    ...
    HistogramEvolutionShim.prototype.range = function (startDate, endDate) {
      ...
      return new TelemetryShim.HistogramShim(this._Telemetry, this._channel,
        this._version, this._measure, this._useSubmissionDate, this._kind,
        this._description, newDates, this._filterNames, this._filterOptionsList,
        this._filters);
    };

    // The functions below need to make synchronous network requests

    HistogramEvolutionShim.prototype.each = function (callback, context) {
    ...
    HistogramEvolutionShim.prototype.map = function (callback, context) {
      ...
      return this._Evolution.map(function (histogram, i, date) {
        var dateString = ("000" + date.getUTCFullYear())
          .substr(-4) + ("0" + (date.getUTCMonth() + 1))
          .substr(-2) + ("0" + date.getUTCDate())
          .substr(-2);
        var histogramShim = new TelemetryShim.HistogramShim(_this._Telemetry,
          _this._channel, _this._version, _this._measure, _this._useSubmissionDate,
          _this._kind, _this._description, [dateString], _this._filterNames,
          _this._filterOptionsList, _this._filters);
    ...
    HistogramEvolutionShim.prototype.getEvolutionSync = function () {
      ...
      this._Evolution = getEvolutionSync(this._Telemetry, this._channel,
        this._version, this._measure, this._filters, this._useSubmissionDate,
        this._dates);
    };
  ...
  TelemetryShim.HistogramShim = (function () {
    function HistogramShim(Telemetry, channel, version, measure,
      useSubmissionDate, kind, description, dates, filterNames,
      filterOptionsList, filters) {
      this._Telemetry = Telemetry;
      this._channel = channel;
    ...
    HistogramShim.prototype.measure = function () {
      return this._measure;
    }
    HistogramShim.prototype.kind = function () {
      return this._kind;
    }
    HistogramShim.prototype.description = function () {
      return this._description;
    }
    HistogramShim.prototype.filterName = function () {
      return this._filterNames[0] || null;
    };
    HistogramShim.prototype.filterOptions = function () {
      return (this._filterOptionsList[0] || [])
        .slice();
    };
    ...
    HistogramShim.prototype.filter = function (option) {
      ...
      newFilters[this.filterName()] = option; // Add the new filter
      return new HistogramShim(this._Telemetry, this._channel, this._version,
        this.measure(), this._useSubmissionDate, this.kind(), this.description(),
        this._dates, this._filterNames.slice(1), this._filterOptionsList
        .slice(1), newFilters);
    };

    // The functions below need to make synchronous network requests

    HistogramShim.prototype.submissions = function () {
      this.getHistogramSync();
      return this._Histogram.submissions;
    };
    HistogramShim.prototype.count = function () {
      this.getHistogramSync();
      return this._Histogram.count;
    };
    HistogramShim.prototype.mean = function () {
      this.getHistogramSync();
      return this._Histogram.mean();
    };
    HistogramShim.prototype.standardDeviation = function () {
      return 0;
    };
    HistogramShim.prototype.geometricMean = function () {
      return 0;
    };
    HistogramShim.prototype.geometricStandardDeviation = function () {
      return 0;
    };
    HistogramShim.prototype.percentile = function (value) {
      this.getHistogramSync();
      return this._Histogram.percentile(value);
    };
    HistogramShim.prototype.median = function (value) {
      this.getHistogramSync();
      return this._Histogram.percentile(50);
    };
    HistogramShim.prototype.each = function (callback, context) {
      this.getHistogramSync();
      this._Histogram.map(callback, context);
    };
    HistogramShim.prototype.map = function (callback, context) {
      this.getHistogramSync();
      return this._Histogram.map(callback, context);
    };
    ...
    HistogramShim.prototype.getHistogramSync = function () {
      ...
      var evolution = getEvolutionSync(this._Telemetry, this._channel,
        this._version, this._measure, this._filters, this._useSubmissionDate,
        this._dates);
