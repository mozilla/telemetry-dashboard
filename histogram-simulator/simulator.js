...
$(function () {
  ...
  var url = window.location.hash[0] === "#" ? window.location.hash.slice(1) :
  ...
  url.split("&")
    .forEach(function (fragment, i) {
      ...
      pageState[decodeURIComponent(parts[0])] = decodeURIComponent(parts[
        1]);
    });
  if (pageState.low) {
    $("input[name=histogram-lower]")
      .val(pageState.low);
  }
  if (pageState.high) {
    $("input[name=histogram-upper]")
      .val(pageState.high);
  }
  if (pageState.n_buckets) {
    $("input[name=histogram-bucket-count]")
      .val(pageState.n_buckets);
  }
  if (pageState.kind) {
    $("#histogram-kind")
      .val(pageState.kind);
  }
  if (pageState.generate) {
    $("#generate-" + pageState.generate)
      .click();
  }

  gDataEditor = CodeMirror.fromTextArea($("#data-editor")
    .get(0), {
      mode: "javascript",
      lineNumbers: true,
    });
  ...
  $(
      "input[name=histogram-lower], input[name=histogram-upper], input[name=histogram-bucket-count]"
    )
    .on("input", deferredUpdate);
  $("#histogram-kind")
    .change(deferredUpdate);
  gDataEditor.on("changes", function () {
    if (gIgnoreGeneration) {
      return;
    }
    $("input[name=histogram-lower]")
      .trigger("input");
  ...
  $("#generate-normal, #generate-log-normal, #generate-uniform")
    .click(function (e) {
      $("#data-editor")
        .next()
        .hide();
      var lower = gCurrentBuckets[0],
        upper = gCurrentBuckets[gCurrentBuckets.length - 1];
      var values = [];
      switch ($(e.target)
        .attr("id")) {
      case "generate-normal": // Generate normally-distributed values using box-muller transform
        values = normalRandoms((lower + upper) / 2, (upper - lower) / 8,
            gSampleCount)
          .map(function (value) {
      ...
      case "generate-log-normal":
        values = logNormalRandoms(Math.sqrt(Math.max(lower, 1) * upper),
            Math.pow(upper / Math.max(lower, 1), 1 / 8), gSampleCount)
          .map(function (value) {
            return value >= 0 ? value : 0;
          });
        break;
      case "generate-uniform":
        for (var i = 0; i < gSampleCount; i++) {
          values.push(Math.random() * (upper - lower) + lower);
        }
        break;
      }
...
function update() {
  var lower = parseInt($("input[name=histogram-lower]")
    .val());
  var upper = parseInt($("input[name=histogram-upper]")
    .val());
  var bucketCount = parseInt($("input[name=histogram-bucket-count]")
    .val());
  if (!isFinite(lower) || !isFinite(upper) || lower < 0 || lower >= upper) {
    $("#data-status")
      .text("Lower bound should be finite, positive, and less than upper bound");
    displayHistogram();
    return;
  }
  if (!isFinite(bucketCount) || bucketCount < 3) {
    $("#data-status")
      .text("Bucket count must be finite and at least 3");
    displayHistogram();
    return;
  }
  if (bucketCount > upper - lower + 2) { // There are 2 additional buckets - one for underflow, and one for overflow
    $("#data-status")
      .text("Bucket count must be at most number of distinct integers in range");
    displayHistogram();
    return;
  }

  var kind = $("#histogram-kind")
    .val();
  $(
      "input[name=histogram-lower], input[name=histogram-upper], input[name=histogram-bucket-count]"
    )
  ...
  if ($("input[name=generate]:checked")
    .attr("id") !== "generate-custom") {
    $("input[name=generate]:checked")
      .click();
  }
  ...
  try {
    ...
    if (!$.isArray(values) || values.length === 0) {
      throw "Values must be array of numbers";
    }
    values.forEach(function (value) {
      if (typeof value !== "number" || value < 0) {
        throw "Values must be array of non-negative numbers";
      }
    });
  } catch (e) {
    $("#data-status")
  ...
  if (kind === "linear" && bucketCount == upper - lower + 2) {
    $("#data-status")
      .text(
        "Consider using enumerated histograms when using individual buckets for each integer."
      );
  }
...
function displayHistogram(counts, starts, ends) {
  ...
  $("#bucket-width")
    .text("Using these parameters, the widest bucket's width is " + maxWidth +
      ".");

  var totalCount = counts.reduce(function (previous, count) {
    return previous + count;
  }, 0);
  var values = counts.map(function (count, i) {
    return {
      value: i,
  ...
  MG.data_graphic({
    ...
    left: 150,
    right: $("#distribution")
      .width() / (values.length + 1) + 150,
    bottom: 90,
    transition_on_update: false,
    target: "#distribution",
    x_label: "Buckets",
    y_label: "Percentage of Samples",
    xax_count: values.length,
    xax_tick_length: 0.5,
    ...
    mouseover: function (d, i) {
      ...
      var label;
      if (starts[d.x] === 0) {
        label = count + " samples (" + percentage +
          ") where sample value < " + formatNumber(ends[d.x]) +
          " (underflow)";
      } else if (ends[d.x] === Infinity) {
        label = count + " samples (" + percentage +
          ") where sample value \u2265 " + formatNumber(starts[d.x]) +
          " (overflow)";
      } else {
        label = count + " samples (" + percentage + ") where " +
          formatNumber(starts[d.x]) + " \u2264 sample value < " +
          formatNumber(ends[d.x]);
      }
      var offset = $("#distribution .mg-bar:nth-child(" + (i + 1) + ")")
        .get(0)
        .getAttribute("transform");
      var barWidth = $("#distribution .mg-bar:nth-child(" + (i + 1) +
          ") rect")
        .get(0)
        .getAttribute("width");

      // Reposition element
      var legend = d3.select("#distribution .mg-active-datapoint")
        .text(label)
  ...
  $(
      "#distribution .mg-x-axis text, .mg-y-axis text, .mg-histogram .axis text, .mg-baselines text, .mg-active-datapoint"
    )
    .css("font-size", "12px");
  $("#distribution .mg-x-axis .label")
    .attr("dy", "2.5em");
  $("#distribution .mg-x-axis text:not(.label)")
...
function saveStateToURL() {
  var hashState = "#low=" + encodeURIComponent($("input[name=histogram-lower]")
      .val()) +
    "&high=" + encodeURIComponent($("input[name=histogram-upper]")
      .val()) +
    "&n_buckets=" + encodeURIComponent($("input[name=histogram-bucket-count]")
      .val()) +
    "&kind=" + encodeURIComponent($("#histogram-kind")
      .val()) +
    "&generate=" + encodeURIComponent($("input[name=generate]:checked")
      .val());
