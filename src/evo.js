...
Telemetry.init(function () {
  ...
  $("#aggregates")
    .multiselect("select", gInitialPageState.aggregates);
  multiselectSetOptions($("#min-channel-version, #max-channel-version"),
  ...
  var nightlyVersions = Telemetry.versions()
  ...
  var versions = Telemetry.versions()
    .filter(function (v) {
      return fromVersion <= v && v <= toVersion;
    });
  ...
  updateMeasuresList(function () {
    calculateHistogramEvolutions(function (filterList,
      filterOptionsList, lines, submissionLines) {
      ...
      $("#measure")
        .change(function () {
          ...
          $("#measure-description")
            .text(measureEntry.description + " (" + measure + ")");
          $("#submissions-title")
            .text(measure + " submissions");

          // Figure out which aggregates actually apply to this measure
          var options = [];
          if (measureEntry.kind == "linear" || measureEntry.kind ==
            "exponential") {
          ...
          } else if (measureEntry.kind === "boolean" ||
            measureEntry.kind === "flag") {
            options = [["mean", "Mean"]];
          }

          var aggregatesFilter = $("#aggregates");
          var oldAggregates = gInitialPageState.aggregates.filter(
            function (aggregate) { // Aggregates that are still available
              return options.reduce(function (contained, option) {
                return contained || option[0] === aggregate
              }, false);
            });
          multiselectSetOptions(aggregatesFilter, options,
            oldAggregates.length > 0 ? oldAggregates : (options
              .length > 0 ? [options[0][0]] : []));
          gPreviousFilterAllSelected[aggregatesFilter.attr("id")] =
            false;

          aggregatesFilter.trigger("change");
        });
      $(
          "input[name=build-time-toggle], input[name=sanitize-toggle], #aggregates, #filter-product, #filter-arch, #filter-os"
        )
        .change(function (e) {
          var $this = $(this);
  ...
  versions.forEach(function (channelVersion) { // Load combined measures for all the versions
    Telemetry.measures(channelVersion, function (measures) {
      ...
      if (versionCount === versions.length) { // All versions are loaded
        indicate();
        var measuresList = Object.keys(gMeasureMap)
          .sort();
...
function refreshFilters(filterList, filterOptionsList) {
  ...
  var optionsList = filterOptionsList.map(function (options) {
    ...
    return options.filter(function (option) {
      if (seen.hasOwnProperty(option[0])) {
        return false;
      }
      seen[option[0]] = true;
      return true;
  ...
  optionsList[2].map(function (entry) {
      return entry[0];
    })
    .sort(function (a, b) {
...
function calculateHistogramEvolutions(callback) {
  ...
  var fromVersion = $("#min-channel-version")
    ...
    .val();
  var measure = $("#measure")
    .val(); // Handle the special case for measures, added in getHumanReadableOptions
  var aggregates = $("#aggregates")
    .val() || [];
  var evolutionLoader = $("input[name=build-time-toggle]:checked")
    .val() !== "0" ? Telemetry.loadEvolutionOverTime : Telemetry.loadEvolutionOverBuilds;
  ...
  for (var filterName in gFilters) {
    ...
    if (selection.length != filterSelector.find("option")
      .length) { // Not all options are selected
  ...
  versions = versions.filter(function (channelVersion) {
    return gVersionMeasureMap[channelVersion][measure] !== undefined;
  });
  ...
  versions.forEach(function (version) {
    evolutionLoader(version, measure, function (histogramEvolution) {
      if (asyncOperationWasInterrupted("calculateHistogramEvolutions",
          operation)) { // Don't call callback if this isn't the latest invocation of the function
        return;
      }

      indicate("Updating evolutions... ", 100 * lines.length /
        expectedCount);

      // Update filter options
      var versionOptionsList = getOptions(filterList,
        histogramEvolution);
      while (filterOptionsList.length < versionOptionsList.length) {
      ...
      var newLines = getHistogramEvolutionLines(version, measure,
        histogramEvolution, aggregates, filterList, $(
...
function getHistogramEvolutionLines(version, measure, histogramEvolution,
  aggregates, filterList, sanitize) {
  ...
  filterList.forEach(function (options, i) {
    ...
    evolutions = [].concat.apply([], evolutions.map(function (evolution) {
      var actualOptions = options,
        fullOptions = evolution.filterOptions();
      if (actualOptions === null) {
        actualOptions = fullOptions;
      }
      ...
      return actualOptions.map(function (option) {
        return evolution.filter(option);
      });
    }));
  });
  ...
  evolutions.forEach(function (evolution) {
    evolution.each(function (date, hgram) {
      ...
      if (firstHistogram === null) {
        ...
        firstFilterId = firstHistogram._dataset[0][firstHistogram._dataset[
          0].length + Telemetry.DataOffsets.FILTER_ID];
      }

      // precomputeAggregateQuantity will perform the actual filtering for us, and then we set the filter ID manually
      var timestamp = date.getTime();
      if (isFinite(timestamp)) {
        if (!dateDatasets.hasOwnProperty(timestamp)) {
          dateDatasets[timestamp] = [];
        }
        var filteredDataset = hgram._dataset[0].map(function (value, i) {
          return hgram.precomputeAggregateQuantity(i);
        });
        filteredDataset[filteredDataset.length + Telemetry.DataOffsets.FILTER_ID] =
          firstFilterId;
  ...
  var aggregateValue = {
    "mean": function (histogram) {
      return histogram.mean();
    },
    "5th-percentile": function (histogram) {
      return histogram.percentile(5);
    },
    "25th-percentile": function (histogram) {
      return histogram.percentile(25);
    },
    "median": function (histogram) {
      return histogram.median();
    },
    "75th-percentile": function (histogram) {
      return histogram.percentile(75);
    },
    "95th-percentile": function (histogram) {
      return histogram.percentile(95);
    },
    "submissions": function (histogram) {
  ...
  aggregates.forEach(function (aggregate) {
    aggregatePoints[aggregate] = [];
  });
  var timeCutoff = moment()
    .add(1, "years")
    .toDate()
    .getTime(); // Cut off all dates past one year in the future
  var maxSubmissions = 0,
    submissionCounts = [];
  Object.keys(dateDatasets)
    .sort()
  ...
  if (sanitize) {
    ...
    for (aggregate in aggregatePoints) {
      aggregatePoints[aggregate] = aggregatePoints[aggregate].filter(function (
        point, i) {
  ...
  var lines = [];
  for (aggregate in aggregatePoints) {
    var newLine = new Line(measure, version, aggregate, aggregatePoints[
      aggregate]);
    newLine.histogramEvolution = histogramEvolution;
    lines.push(newLine);
  }

  var submissionLine = new Line(measure, version, "submissions",
    submissionPoints);
...
function displayEvolutions(lines, submissionLines, minDate, maxDate,
  ...
  MG.data_graphic({
    ...
    linked: true,
    min_x: minDate === null ? null : moment.utc(minDate)
      .add(timezoneOffsetMinutes, "minutes")
      .toDate(),
    max_x: maxDate === null ? null : moment.utc(maxDate)
      .add(timezoneOffsetMinutes, "minutes")
      .toDate(),
    mouseover: function (d, i) {
      ...
      var legendLabel = moment(date)
        .format("dddd MMMM D, YYYY") + (useSubmissionDate ? ":" :
          " (build " + moment(date)
          .format("YYYYMMDD") + "):");
