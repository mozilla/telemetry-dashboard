...
Telemetry.init(function () {
  ...
  multiselectSetOptions($("#channel-version"), getHumanReadableOptions(
    "channelVersion", Telemetry.versions()));
  ...
  $("input[name=table-toggle][value=" + (gInitialPageState.table !== 0 ? 1 :
      0) + "]")
    .prop("checked", true)
    .trigger("change");
  $("input[name=cumulative-toggle][value=" + (gInitialPageState.cumulative !==
    ...
    .prop("checked", true)
    .trigger("change");
  $("input[name=trim-toggle][value=" + (gInitialPageState.trim !== 0 ? 1 :
      0) + "]")
    .prop("checked", true)
    .trigger("change");
  ...
  updateMeasuresList(function () {
    calculateHistogram(function (filterList, filterOptionsList,
      histogram, dates) {
      multiselectSetOptions($("#filter-product"), filterOptionsList[
        1]);
      ...
      $(
          "input[name=build-time-toggle], #measure, #filter-product, #filter-arch, #filter-os"
        )
        .change(function () {
          var $this = $(this);
          ...
          gFilterChangeTimeout = setTimeout(function () { // Debounce the changes to prevent rapid filter changes from causing too many updates
            ...
            calculateHistogram(function (filterList,
              filterOptionsList, histogram, dates) {
              refreshFilters(filterOptionsList);

              // Update the measure description
              var measureDescription = gMeasureMap[$(
                  "#measure")
                .val()].description;
              gCurrentDates = dates;
              gCurrentHistogram = histogram;
  ...
  $(
    ...
    .change(function () {
      displayHistogram(gCurrentHistogram, gCurrentDates, $(
          "input[name=table-toggle]:checked")
        .val() !== "0", $("input[name=cumulative-toggle]:checked")
...
function updateMeasuresList(callback) {
  ...
  Telemetry.measures(channelVersion, function (measures) {
    ...
    indicate();
    var measuresList = Object.keys(measures)
      .sort()
  ...
  evolutionLoader(channelVersion, measure, function (histogramEvolution) {
    ...
    updateDateRange(function (dates) {
      ...
      if (dates === null) {
        ...
        var fullHistogram = histogramEvolution.range(dates[0], dates[
          dates.length - 1]);
        var filteredHistogram = getFilteredHistogram(channelVersion,
          measure, fullHistogram, filters, filterList);
        callback(filterList, filterOptionsList, filteredHistogram,
          dates);
      }
...
function updateDateRange(callback, evolution, updatedByUser,
  ...
  var timezoneOffsetMinutes = (new Date)
    .getTimezoneOffset(); // All moments and dates are UTC mode where possible, and should have 00:00:00 as their time
  var dates = [];
  if (evolution !== null) {
  ...
  picker.setOptions({
    ...
    ranges: {
      ...
      "Last 30 Days": [moment.utc(maxMoment)
        .subtract(30, "days")
        .format("YYYY-MM-DD"), endMoment],
      "Last 7 Days": [moment.utc(maxMoment)
        .subtract(6, "days")
        .format("YYYY-MM-DD"), endMoment],
    },
  ...
  if (!gLoadedDateRangeFromState && gInitialPageState.start_date !== null &&
    ...
    if (gInitialPageState.use_submission_date !== 0 || gInitialPageState.table !==
      ...
      startMoment !== moment.utc(fullDates[0])
      .format("YYYY-MM-DD") || endMoment !== moment.utc(fullDates[fullDates.length -
        1])
      .format("YYYY-MM-DD")) {
  ...
  if (pickerStartDate > maxMoment || pickerStartDate < minMoment ||
    ...
    (!updatedByUser && (minMoment !== gPreviousMinMoment || maxMoment !==
      gPreviousMaxMoment))) {
    picker.setStartDate(moment(minMoment));
    picker.setEndDate(moment(maxMoment));
    ...
    var rangeBarControl = RangeBar({
      ...
      .on("changing", function (e, ranges, changed) {
        ...
        gLastTimeoutID = setTimeout(function () { // Debounce slider movement callback
          picker.setStartDate(moment(moment.utc(range[0])
            .format("YYYY-MM-DD")))
          picker.setEndDate(moment(moment.utc(range[1])
            .subtract(1, "days")
            .format("YYYY-MM-DD")));
          updateDateRange(gCurrentDateRangeUpdateCallback, evolution,
            true, false);
        }, 50);
      });
    $("#range-bar")
...
function getFilteredHistogram(version, measure, histogram, filters, filterList) {
  ...
  filterList.forEach(function (options, i) {
    ...
    histograms = [].concat.apply([], histograms.map(function (histogram) {
      var actualOptions = options,
        fullOptions = histogram.filterOptions();
  ...
  var firstFilterId = histogram._dataset[0][histogram._dataset[0].length +
    Telemetry.DataOffsets.FILTER_ID];
  ...
  return new Telemetry.Histogram(measure, histogram._filter_path, histogram._buckets,
...
function displayHistogram(histogram, dates, useTable, cumulative, trim) {
  ...
  if (histogram === null || histogram._buckets.length < 2 || histogram.count() ===
    0) {
    $("#summary")
      .hide();
    MG.data_graphic({
  ...
  $("#prop-date-range")
    .text(
      moment.utc(dates[0])
      .format("YYYY/MM/DD") +
      ((dates.length == 1) ? "" : " to " + moment.utc(dates[dates.length - 1])
        .format("YYYY/MM/DD"))
    );
  $("#prop-submissions")
  ...
  if (histogram.kind() == "linear" || histogram.kind() == "exponential") {
    ...
    $("#prop-p50")
      .text(formatNumber(histogram.median()));
    $("#prop-p75")
  ...
  var starts = histogram.map(function (count, start, end, i) {
    return start;
  });
  ...
  var ends = histogram.map(function (count, start, end, i) {
    return end;
  });
  ...
  MG.data_graphic({
    ...
    left: 150,
    right: $(axes)
      .width() / (distributionSamples.length + 1),
    ...
    mouseover: function (d, i) {
      var count = formatNumber(counts[d.x]),
      ...
      if (x + bbox.width / 2 > $(axes)
        .find("svg")
        .width() - 5) {
        offset = "translate(" + ($(axes)
          .find("svg")
          .width() - 5 - bbox.width / 2 - 10) + ",60)";
        legend.attr("transform", offset);
      }
    ...
    mouseout: function (d, i) {
      d3.select("#distribution .active-datapoint-background")
        .remove(); // Remove old background
    },
  });
  ...
  var barWidth = parseFloat($(
      "#distribution .mg-rollover-rects:last-child rect")
    .attr("width"))
  $("#distribution .mg-extended-y-ticks")
          ...
          .append(
            counts.map(function (count, i) {
              ...
              [
                ...
                $("<td></td>")
                    .text(formatNumber(counts[i])),
                $("<td></td>")
                    .text(Math.round(10000 * counts[i] / histogram.count()) /
                      100 + "%"),
              ]
                )
            })
...
function saveStateToUrlAndCookie() {
    ...
    if ($("input[name=cumulative-toggle]:checked")
      ...
      jsonHistogram = gCurrentHistogram.map(function (count, start, end, i) {
        ...
        return {
          start: start,
          count: total,
          percentage: 100 * total / gCurrentHistogram.count()
        };
      });
    } else {
      jsonHistogram = gCurrentHistogram.map(function (count, start, end, i) {
        return {
          start: start,
          count: count,
          percentage: 100 * count / gCurrentHistogram.count()
        }
      });
    }
    var csvValue = "start,\tcount,\tpercentage\n" + jsonHistogram.map(function (
        entry) {
  ...
  if (gCurrentEvolution !== null) {
    ...
    var startMoment = moment.utc(fullDates[0])
      .format("YYYY-MM-DD"),
      endMoment = moment.utc(fullDates[fullDates.length - 1])
      .format("YYYY-MM-DD");
  } else {
